...
public class BlockModelGenerator extends BlockModelProvider {
    ...
    private void sofa(String color) {
        Identifier texture = modLoc("block/deco/sofa/%s".formatted(color));
        Identifier particle = mcLoc("block/%s_wool".formatted(color));

        for (ConnectionType type : ConnectionType.values()) {
            String typeName = type.getSerializedName();
            String name = "block/deco/sofa/%s/%s".formatted(color, typeName);
            Identifier parent = modLoc("block/deco/sofa/base/%s".formatted(typeName));
    ...
    private void barStool(String color) {
        Identifier texture = modLoc("block/deco/bar_stool/%s".formatted(color));
        Identifier particle = mcLoc("block/%s_wool".formatted(color));
    ...
    private void sandwichBoard(String type) {
        Identifier texture = modLoc("block/deco/sandwich_board/%s".formatted(type));
    ...
    private void painting(String type) {
        Identifier texture = modLoc("block/deco/painting/%s".formatted(type));
